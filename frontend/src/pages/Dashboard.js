...
const Dashboard = () => {
  ...
  const [top3Centres, setTop3Centres] = useState({
    data: [],
    loading: true
  });
    ...
    const loadStats = async () => {
      try {
        const results = await Promise.allSettled([
          ...
          kpiService.getTop3CentresMonth()
        ]);

        const [usersRes, centresCountRes, agencesCountRes, communesCountRes, highestRateRes, highestCentreRateRes, highestMonthlyAvgRes, highestMonthlyCentreAvgRes, top3AgencesRes, top3CentresRes] = results;
        ...
        if (top3AgencesRes.status === 'rejected') {
          console.error('❌ Erreur lors du chargement du top 3 agences:', top3AgencesRes.reason);
        }
        ...
        const top3CentresData = top3CentresRes.status === 'fulfilled' ? top3CentresRes.value : null;
        if (top3CentresRes.status === 'rejected') {
          console.error('❌ Erreur lors du chargement du top 3 centres:', top3CentresRes.reason);
        }
        if (top3CentresData && Array.isArray(top3CentresData)) {
          console.log('✅ Dashboard - Top 3 centres reçues:', top3CentresData);
          setTop3Centres({
            data: top3CentresData,
            loading: false
          });
        } else {
          console.log('⚠️ Dashboard - Aucun top 3 centres trouvé, données reçues:', top3CentresData);
          setTop3Centres({ data: [], loading: false });
        }
      } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-sky-100 dark:from-slate-900 dark:to-slate-950 text-gray-800 dark:text-slate-100 font-sans">
      ...
      <div className="w-full">
        ...
        <main className="p-6 fade-in space-y-6">
          ...
          <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
            {/* Podium Top 3 Centres - Carte agrandie (2 colonnes) */}
            <div className="w-full col-span-1 sm:col-span-2 lg:col-span-2 min-h-[160px] p-1 bg-gradient-to-br from-emerald-400 via-sky-200 to-cyan-100 dark:!bg-slate-800 dark:[background-image:none] dark:border-sky-500/50 rounded-2xl shadow-xl border-4 border-white dark:border-2 hover:shadow-2xl hover:scale-105 transition-all duration-200 flex flex-col justify-between">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs uppercase tracking-wide font-semibold text-gray-700/90 dark:text-slate-200 p-1">Top 3 Centres</p>
              ...
              {top3Centres.loading ? (
                ...
                <div className="flex items-end justify-center space-x-4 md:space-x-8">
                    ...
                    {top3Centres.data[1] ? (
                      <>
                        <div className="text-sm font-semibold text-gray-800 dark:text-gray-100 text-center px-1 max-w-[120px] truncate">
                          {top3Centres.data[1].Nom_Centre || 'N/A'}
                        </div>
                        <div className="text-base font-bold text-gray-600 dark:text-gray-300">
                          {typeof top3Centres.data[1].Taux_Mensuel === 'number' ? top3Centres.data[1].Taux_Mensuel.toFixed(2) + '%' : '-'}
                        </div>
                      </>
                    ) : (
                    ...
                    {top3Centres.data[0] ? (
                      <>
                        <div className="text-sm font-semibold text-yellow-900 dark:text-yellow-100 text-center px-1 max-w-[130px] truncate">
                          {top3Centres.data[0].Nom_Centre || 'N/A'}
                        </div>
                        <div className="text-base font-bold text-yellow-600 dark:text-yellow-400">
                          {typeof top3Centres.data[0].Taux_Mensuel === 'number' ? top3Centres.data[0].Taux_Mensuel.toFixed(2) + '%' : '-'}
                        </div>
                      </>
                    ) : (
                  ...
                  <div className="flex flex-col items-center space-y-2">
                    ...
                    {top3Centres.data[2] ? (
                      <>
                        <div className="text-sm font-semibold text-amber-900 dark:text-amber-200 text-center px-1 max-w-[120px] truncate">
                          {top3Centres.data[2].Nom_Centre || 'N/A'}
                        </div>
                        <div className="text-base font-bold text-amber-700 dark:text-amber-400">
                          {typeof top3Centres.data[2].Taux_Mensuel === 'number' ? top3Centres.data[2].Taux_Mensuel.toFixed(2) + '%' : '-'}
                        </div>
                      </>
                    ) : (
